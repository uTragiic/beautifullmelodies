...
class DataManager:
    ...
    def _load_symbols(self, symbols: List[str], start_date: str, end_date: str):
        ...
        if symbols_to_load:
            batch_data = self.db_handler.load_all_market_data(
                symbols=symbols_to_load, start_date=start_date, end_date=end_date
            )
    ...
    def _move_to_disk(self):
        ...
        symbols_to_move = [
            s for s in list(self._data_cache.keys())[:items_to_move] if s != "SPY"
        ]
        ...
        for symbol in symbols_to_move:
            ...
            mmap_array = np.memmap(
                mmap_path, dtype="float64", mode="w+", shape=data.values.shape
            )
            ...
            with open(metadata_path, "wb") as f:
                pickle.dump(
                    {"index": data.index, "columns": data.columns, "shape": data.shape},
                    f,
                )
    ...
    def _get_cached_data(self, symbol: str) -> pd.DataFrame:
        ...
        if symbol in self._memory_mapped_files:
            ...
            mmap_array = np.memmap(
                mmap_path, dtype="float64", mode="r", shape=metadata["shape"]
            )

            # Reconstruct DataFrame
            return pd.DataFrame(
                mmap_array, index=metadata["index"], columns=metadata["columns"]
            )
...
class TrainingManager:
    ...
    def __init__(self, config_path: str):
        ...
        self.data_manager = DataManager(
            self.db_handler, self.cache_dir, self.memory_threshold
        )
    ...
    def _init_components(self):
        ...
        self.universe_manager = UniverseManager(
            ...
            config_path=self.config.get(
                "universe_config_path", "project/config/universe_config.json"
            ),
    ...
    def _train_model_parallel(
        ...
        try:
            ...
            # Validate the model
            model = (
                self.model_trainer.sector_models[identifier]
                if model_type == "sector"
            ...
            is_overfit, overfitting_scores = (
                self.overfitting_controller.detect_overfitting(
            ...
            return {"type": model_type, "identifier": identifier, "error": str(e)}
    ...
    def train_models(self):
        ...
        try:
            ...
            logger.info(
                f"Loading market data for {len(tradable_symbols)} tradable symbols"
            )

            # Load data efficiently using DataManager
            all_market_data = self.data_manager.get_market_data(
                symbols=tradable_symbols, start_date=start_date, end_date=end_date
            )
            ...
            market_results = self.model_trainer._train_market_model(
                self.config["training"]["start_dates"]["market"], end_date
            )
    ...
    def _prepare_consolidated_data(self, market_data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        try:
            ...
            missing_columns = [
                col for col in required_columns if col not in consolidated.columns
            ]
            if missing_columns:
                raise ValueError(
                    f"Missing required columns in consolidated data: {missing_columns}"
                )

            logger.info(
                f"Successfully prepared consolidated data with {len(consolidated)} rows"
            )
            return consolidated
    ...
    def _organize_training_results(
        ...
        for result in training_results:
            if result.get("error"):
                logger.error(
                    f"Error in {result['type']} model {result['identifier']}: {result['error']}"
                )
                continue
    ...
    def _update_training_metrics(self, result: Dict[str, Any]):
        ...
        if "type" in result and "identifier" in result:
            self.training_metrics["training_progress"][
                f"{result['type']}_{result['identifier']}"
            ] = "completed"
            if "metrics" in result:
                self.training_metrics["validation_metrics"][
                    f"{result['type']}_{result['identifier']}"
                ] = result["metrics"]
        ...
        report = {
            ...
            "model_performance": {
                "market_model": self._analyze_model_performance(
                    results["market_model"]["metrics"], "market"
                ),
                "sector_models": {
                    sector: self._analyze_model_performance(
                ...
                "cluster_models": {
                    sector: {
                        cluster: self._analyze_model_performance(
    ...
    def _identify_problematic_models(
        ...
        # Check market model
        check_model(results["market_model"]["metrics"], "market", "market")
        ...
        for sector, clusters in results["cluster_models"].items():
            for cluster, data in clusters.items():
                check_model(data["metrics"], "cluster", f"{sector}/{cluster}")
