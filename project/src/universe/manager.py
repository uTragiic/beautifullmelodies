...
class UniverseManager:
    ...
    def __init__(self, 
        ...
        self.regime_history = []
        self.regime_lookback = self.config.get('regime_lookback', 200)  # Minimum needed for SMA200
    ...
    def _calculate_current_market_regime(self) -> str:
        ...
        try:
            ...
            # Classify volatility
            vol_high = spy_data['Volatility'].quantile(self.regime_thresholds['volatility_high_quantile'])
            vol_low = spy_data['Volatility'].quantile(self.regime_thresholds['volatility_low_quantile'])
            ...
            # Classify volume
            if current['volume'] > current['volume_MA'] * self.regime_thresholds['volume_high_mult']:
                volume = 'High'
            elif current['volume'] < current['volume_MA'] * self.regime_thresholds['volume_low_mult']:
                volume = 'Low'
            else:
                volume = 'Normal'
            ...
            regime = f"{trend}-{volatility}_Volatility-{volume}_Volume-{momentum}_Momentum"
    ...
    def build_universe(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            self._calculate_stock_features(['SPY'] + [s for s in tradable_symbols if s != 'SPY'])
            self.clusters = self._group_by_sector(tradable_symbols)
            
            # Cache results
            self._cache_universe(self.clusters)
            
            logger.info(f"Universe built with {len(tradable_symbols)} symbols (including SPY)")
    ...
    def _apply_filters(self, symbols: List[str]) -> List[str]:
        ...
        try:
            ...
            logger.info(f"Filtering complete. {len(passed_symbols)} symbols passed (including SPY)")
    ...
    def _check_symbol_criteria_debug(self, symbol: str, **filters) -> dict:
        ...
        try:
            ...
            # Basic checks
            checks = {
                'history': (len(data) >= filters['min_history_days'], 
                          f"Only {len(data)} days of history"),
                'price_range': (filters['min_price'] <= latest_price <= filters['max_price'],
                              f"Price {latest_price:.2f} outside range"),
                'volume': (avg_volume >= filters['min_volume'],
                         f"Volume {avg_volume:.0f} below minimum"),
            }
            
            # Add failures for failed checks
            failures.update({key: msg for key, (check, msg) in checks.items() if not check})
    ...
    def _group_by_sector(self, symbols: List[str]) -> Dict[str, List[str]]:
        ...
        try:
            ...
            volume_threshold = features_df['avg_volume'].median() if 'avg_volume' in features_df else features_df['volume'].median()
            ...
            for symbol in symbols:
                ...
                if metrics['market_cap'] > market_cap_thresholds[0.67]:
                    target_group = 'Large_Cap_High_Vol' if is_high_volume else 'Large_Cap_Low_Vol'
                elif metrics['market_cap'] > market_cap_thresholds[0.33]:
                    target_group = 'Mid_Cap_High_Vol' if is_high_volume else 'Mid_Cap_Low_Vol'
                else:
                    target_group = 'Small_Cap_High_Vol' if is_high_volume else 'Small_Cap_Low_Vol'
